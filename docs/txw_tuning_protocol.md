...
## 9. 实机联调结论（2026-09-16, 板子已连通, 全链路验证通过）

### 已验证
- **写参+ACK**: `AE_LUMA_TARGET=110` → ACK=0；后续 70/170 亮度实测 108.9/206.5(+97.6)——参数实时生效实证
- **包头 CRC**: `crc16(hdr[:10]) == h[5]` 实测匹配
- **命令帧 CRC**: 固件按同款 CRC16-MODBUS 校验我方帧(ACK=0 证明)
- **GET_IMG**: 16字节头 + JPEG 回传, 1080p 解码成功

### 与头文件假设的差异（实机为准）
1. **分帧**: 头文件定义"2048 含尾部", 实机为**每 2048 字节数据 + 4 字节尾部**(2052/包)。按 2044 剥离会得到可解码但有坏字节的 JPEG(容错解码掩盖), 按 2048 剥离零警告——**分块必须是 2048**
2. **包尾 4 字节语义未定**: 非 `[CRC16][0x000A]`, 暴力搜索 2000~2048 所有分块 × 两种尾部序 × 两种 CRC 初值均不匹配 → 跳过校验
